    ...
    async def update_detections(self, detections_json: str):
        ...
        try:
            ...
            if "detection_count" not in self.vars:
                ...
                self.vars["detection_count"] = await detection_obj.add_variable(
                    self.idx, "Count", 0
                )
    ...
    async def update_video_status(self, healthy: bool, fps: float, recording: bool):
        ...
        try:
            ...
            if "video_healthy" not in self.vars:
                ...
                self.vars["video_healthy"] = await video_obj.add_variable(
                    self.idx, "Healthy", False
                )
                self.vars["video_fps"] = await video_obj.add_variable(
                    self.idx, "FPS", 0.0
                )
                self.vars["video_recording"] = await video_obj.add_variable(
                    self.idx, "Recording", False
                )
    ...
    async def update_range(self, est_range_km: float, feasible: bool, reason: str):
        ...
        try:
            ...
            if "range_km" not in self.vars:
                ...
                self.vars["range_km"] = await range_obj.add_variable(
                    self.idx, "EstimatedRangeKm", 0.0
                )
                self.vars["range_feasible"] = await range_obj.add_variable(
                    self.idx, "Feasible", False
                )
