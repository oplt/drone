...
class LLMAnalyzer:
    ...
    def __init__(
        ...
        if not self._is_configured:
            logging.warning(
                "LLM API not configured: missing api_base or model. Object detection will be disabled."
            )
    ...
    def _check_circuit_breaker(self) -> bool:
        ...
        if self._circuit_open:
            if current_time - self._circuit_open_time >= self._circuit_timeout:
                logging.info(
                    f"Circuit breaker: Attempting to close circuit after {self._circuit_timeout}s timeout"
                )
                self._circuit_open = False
                self._failure_count = 0
                return True
                ...
                logging.debug(
                    f"Circuit breaker: Circuit is OPEN (opened {current_time - self._circuit_open_time:.1f}s ago)"
                )
                return False
    ...
    async def detect_objects(self, frame) -> List[Detection]:
        ...
        try:
            ...
            if not was_open and self._circuit_open:
                logging.warning(
                    f"Circuit breaker opened: Cannot connect to Ollama at {self.api_base}. "
            ...
            if not was_open and self._circuit_open:
                logging.warning(
                    f"Circuit breaker opened: Ollama at {self.api_base} is timing out. "
                    f"API calls will be blocked for {self._circuit_timeout}s."
                )
            # Don't log on first failure or if circuit was already open
            return []  # Return empty list instead of raising
            ...
            if not was_open and self._circuit_open:
                ...
                logging.warning(
                    f"Circuit breaker: Opening circuit after {self._failure_count} consecutive failures. "
    ...
    )  # Reduced retries since we have circuit breaker
