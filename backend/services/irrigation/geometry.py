...
def latlon_to_local_xy(*, lat: float, lon: float, origin_lat: float, origin_lon: float) -> tuple[float, float]:
    return (
...
def local_xy_to_latlon(*, x_m: float, y_m: float, origin_lat: float, origin_lon: float) -> tuple[float, float]:
...
def polygon_area_m2(points: list[tuple[float, float]]) -> float:
    ...
    xy = [latlon_to_local_xy(lat=lat, lon=lon, origin_lat=origin_lat, origin_lon=origin_lon) for lat, lon in points]
    area_twice = 0.0
