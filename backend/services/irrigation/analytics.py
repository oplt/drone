...
from backend.services.irrigation.geometry import clamp01, local_xy_to_latlon, polygon_area_m2
...
def _bbox_polygon_for_cluster(
    *,
    ...
    polygon = [
        local_xy_to_latlon(x_m=min_local_x, y_m=min_local_y, origin_lat=origin_lat, origin_lon=origin_lon),
        local_xy_to_latlon(x_m=max_local_x, y_m=min_local_y, origin_lat=origin_lat, origin_lon=origin_lon),
        local_xy_to_latlon(x_m=max_local_x, y_m=max_local_y, origin_lat=origin_lat, origin_lon=origin_lon),
        local_xy_to_latlon(x_m=min_local_x, y_m=max_local_y, origin_lat=origin_lat, origin_lon=origin_lon),
        local_xy_to_latlon(x_m=min_local_x, y_m=min_local_y, origin_lat=origin_lat, origin_lon=origin_lon),
    ]
    ...
    *,
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            saturation_grid[row, col] = float(np.mean(saturation[row_slice, col_slice][patch_valid]))
    ...
    under_score[valid_values] = np.maximum(-exg_z[valid_values], 0.0) + np.maximum(bright_z[valid_values], 0.0) * 0.35
    over_score[valid_values] = np.maximum(-bright_z[valid_values], 0.0) + np.maximum(-sat_z[valid_values], 0.0) * 0.25
    for row in range(rows):
        for col in range(cols):
            if not valid_grid[row, col]:
                continue
            uneven_score[row, col] = abs(float(row_z[row])) * 0.6 + abs(float(col_z[col])) * 0.4 + abs(float(exg_z[row, col])) * 0.25
    ...
    summary = {
        ...
        "counts_by_type": {
            ...
            "uneven_distribution": sum(1 for zone in zones if zone["type"] == "uneven_distribution"),
        },
