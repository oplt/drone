...
from backend.db.models import FieldDeliverable
...
async def get_deliverable_share(token: str):
    ...
    async with Session() as db:
        q = await db.execute(
            select(FieldDeliverable).where(FieldDeliverable.share_token == token)
        )
        ...
        if settings.storage_backend == "s3":
            ...
            if not path.exists():
                logger.error(
                    "Deliverable %s has status=ready but file missing at %s", d.id, path
                )
