...
from backend.db.models import AuthAuditLog, Organization, User
from backend.db.session import Session
from backend.services.access_control import ensure_user_workspace, get_default_project
...
async def login(
    ...
    if not user or not verify_password(payload.password, user.hashed_password):
        async with Session() as audit_db:
            await _write_audit(audit_db, user_id=None, org_id=None, event="login_failed", request=request)
...
async def oidc_google_start():
    ...
    from backend.auth.oidc import google_oidc
...
async def oidc_google_callback(
    ...
    if not settings.google_client_id:
        raise HTTPException(status_code=501, detail="Google OIDC not configured")
    from backend.auth.oidc import get_or_create_sso_user, google_oidc
