...
from backend.ros2_irrigation.common import (
    DroneStateSample,
    active_mission_id,
    json_message,
    mission_spool_root,
...
def _encoding_to_image(message) -> np.ndarray:
    ...
    frame = np.frombuffer(message.data, dtype=np.uint8).reshape((message.height, message.width, channels))
...
def main() -> None:
    ...
    class CaptureSyncNode(Node):
        ...
        def on_state(self, message: String) -> None:
            ...
            timestamp_sec = float(timestamp_value) if isinstance(timestamp_value, (int, float)) else self.get_clock().now().nanoseconds / 1_000_000_000.0
            self.state_buffer.append(
                DroneStateSample(
                    ...
                    yaw_deg=float(payload["yaw_deg"]) if payload.get("yaw_deg") is not None else None,
                    pitch_deg=float(payload["pitch_deg"]) if payload.get("pitch_deg") is not None else None,
                    roll_deg=float(payload["roll_deg"]) if payload.get("roll_deg") is not None else None,
                    waypoint_seq=int(payload["waypoint_seq"]) if payload.get("waypoint_seq") is not None else None,
        ...
        def on_image(self, message: Image) -> None:
            ...
            response = post_capture(mission_id=mission_id_value, payload=payload, image_path=image_path)
