...
from backend.db.models import Animal, AnimalPosition, Geofence, Herd, HerdTask
...
class LivestockRepository:
    ...
    async def get_animal_by_collar(self, db: AsyncSession, *, collar_id: str) -> Animal | None:
        res = await db.execute(select(Animal).where(Animal.collar_id == collar_id))
        return res.scalar_one_or_none()

    async def list_animals(
        self, db: AsyncSession, herd_id: int | None = None, limit: int = 200
    ) -> list[Animal]:
        stmt = select(Animal)
    ...
    async def latest_positions_for_herd(
        self, db: AsyncSession, *, herd_id: int
    ) -> list[dict[str, Any]]:
        """
    ...
    # ---------- Pasture polygon ----------
    async def get_pasture_geofence(self, db: AsyncSession, geofence_id: int) -> Geofence | None:
