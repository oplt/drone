...
def observation_type(label: str) -> str:
    normalized = label.lower().replace("-", "_").replace(" ", "_")
    if any(token in normalized for token in ("gap", "skip", "double", "overcrowd", "discontinu")): return "emergence_issue"
    if any(token in normalized for token in ("weed", "vegetation")): return "weed"
    if any(token in normalized for token in ("water", "flood", "drain")): return "standing_water"
    if any(token in normalized for token in ("plant", "crop", "stand")): return "stand_count"
    if any(token in normalized for token in ("stress", "anomaly", "disease")): return "abnormal_crop_health_signature"
...
def _local_geometry(points: list[tuple[float, float]], radius_m: float = 2.0) -> tuple[dict[str, Any], float]:
    if not points:
        return {}, 0.0
    ...
    shape = MultiPoint(local).convex_hull
    if shape.geom_type == "Point": shape = shape.buffer(radius_m)
    elif shape.geom_type == "LineString": shape = shape.buffer(radius_m)
    else: shape = shape.buffer(radius_m)
    geo = mapping(shape)
...
def aggregate_detections(detections: Iterable[Any], *, cluster_radius_m: float = 8.0) -> list[dict[str, Any]]:
    groups: list[dict[str, Any]] = []
    for detection in sorted(detections, key=lambda row: (float(row.timestamp_seconds), str(row.id))):
        kind = observation_type(str(detection.label))
        point = (float(detection.lon), float(detection.lat)) if detection.lon is not None and detection.lat is not None else None
        track_id = getattr(detection, "track_id", None)
        track_key = str(track_id) if track_id is not None else None
        candidates = [
            group for group in groups
            if group["kind"] == kind and (
                track_key is not None and track_key in group["track_ids"]
                or point is not None and group["points"] and min(_distance_m(point, item) for item in group["points"]) <= cluster_radius_m
            )
        ]
        ...
        if group is None:
            ...
            groups.append(group)
        if point is not None: group["points"].append(point)
        if track_key is not None: group["track_ids"].add(track_key)
    ...
    for group in groups:
        ...
        output.append({
            ...
            "first_detected": min(timestamps), "last_detected": max(timestamps), "trend": "current",
            "evidence_ids": [str(row.id) for row in rows], "sensor_values": {},
            "model_version": next((str((getattr(row, "raw", {}) or {}).get("model_version")) for row in rows if (getattr(row, "raw", {}) or {}).get("model_version")), None),
        })
    return sorted(output, key=lambda row: (-row["severity"], row["observation_type"], row["evidence_ids"][0]))
