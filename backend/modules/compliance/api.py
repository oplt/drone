...
from backend.modules.compliance.models import ComplianceRecord
...
async def _get_runtime(flight_id: str, db) -> MissionRuntime:
    q = await db.execute(select(MissionRuntime).where(MissionRuntime.client_flight_id == flight_id))
...
async def _resolve_preflight_ack(runtime: MissionRuntime, db) -> datetime | None:
    ...
    if not runtime.preflight_run_id:
        return None
    q = await db.execute(
        select(PreflightRun.completed_at).where(PreflightRun.id == runtime.preflight_run_id)
    )
...
async def upsert_compliance(
    ...
    async with Session() as db:
        ...
        q = await db.execute(
            select(ComplianceRecord).where(ComplianceRecord.mission_runtime_id == runtime.id)
        )
