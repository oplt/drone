...
from backend.modules.fleet.models import DeviceReadiness, OperatorCertification
...
async def patch_device_readiness(
    ...
    async with Session() as db:
        q = await db.execute(select(DeviceReadiness).where(DeviceReadiness.id == device_id_pk))
        device = q.scalar_one_or_none()
        if not device:
