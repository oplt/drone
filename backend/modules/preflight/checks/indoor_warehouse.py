    ...
    async def check_local_position_source(self) -> list[CheckResult]:
        ...
        return [
            self._fail(
                ...
                "Indoor mission requires a live local position source",
            )
        ]
        ...
        if value is None:
            ...
            if ceiling_distance is not None:
                return [
                    self._ok(
                        "Indoor Rangefinder",
                        f"Altitude source available ({float(ceiling_distance):.2f}m)",
                    )
                ]
        return self._required_bool_check(
    ...
    async def check_slam_pipeline(self) -> list[CheckResult]:
        ...
        if slam_ready is False or slam_tracking_ok is False:
            return [
                self._fail(
                    ...
                    "SLAM / localization pipeline is not healthy",
                )
            ]
        ...
        if self._value("local_position_ok") is True and self._value("lidar_healthy") is True:
            return [
                self._ok(
                    "Indoor SLAM Pipeline",
                    "Local position and LiDAR imply indoor localization is available",
                )
            ]
    ...
    async def check_dock_reference(self) -> list[CheckResult]:
        ...
        return [
            self._ok(
                ...
                "Dock reference provided in mission configuration",
            )
        ]

    async def check_takeoff_bubble(self) -> list[CheckResult]:
        required_clearance = float(getattr(self.ctx.mission, "safe_takeoff_bubble_radius_m", 1.5))
        ...
        *,
        ...
        return [
            ...
            CheckSpec(
                ...
                Priority.CRITICAL,
                True,
                lambda: self.check_estimator_ready(),
            ),
            CheckSpec(
                "Indoor Local Position",
                Priority.CRITICAL,
                True,
                lambda: self.check_local_position_source(),
            ),
            CheckSpec(
                "Battery",
                Priority.CRITICAL,
            ...
            CheckSpec(
                ...
                Priority.CRITICAL,
                True,
                lambda: self.check_lidar_stream(),
            ),
            CheckSpec(
                "Indoor Rangefinder",
                Priority.CRITICAL,
                True,
                lambda: self.check_rangefinder_health(),
            ),
            CheckSpec(
                "Indoor SLAM Pipeline",
                Priority.CRITICAL,
                True,
                lambda: self.check_slam_pipeline(),
            ),
            CheckSpec(
                "Indoor Dock Reference",
                Priority.CRITICAL,
                True,
                lambda: self.check_dock_reference(),
            ),
            CheckSpec(
                "Indoor Takeoff Bubble",
                Priority.CRITICAL,
                True,
                lambda: self.check_takeoff_bubble(),
            ),
            CheckSpec(
                "Indoor Proximity",
                Priority.SAFETY,
                True,
                lambda: self.check_proximity_health(),
            ),
