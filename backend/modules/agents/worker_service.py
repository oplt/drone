"""Application service for agent worker execution."""
...
from backend.modules.property_patrol.models import PropertyPatrolIncident, PropertyPatrolTemplate


async def _hydrate_context(
    db: Any, agent_id: MissionAgentId, context: AgentContext
) -> AgentContext:
    payload = dict(context.structured_payload)
    if agent_id == MissionAgentId.PRIVATE_PATROL and payload.get("patrol_incident_id"):
        payload.update(
            await build_patrol_incident_context(db, incident_id=int(payload["patrol_incident_id"]))
        )
...
async def execute_agent(agent_id: MissionAgentId, context: AgentContext) -> dict[str, Any]:
...
async def summarize_property_patrol_incident(*, incident_id: int) -> dict[str, Any]:
    async with Session() as db:
    ...
    return await execute_agent(MissionAgentId.PROPERTY_PATROL, context)
