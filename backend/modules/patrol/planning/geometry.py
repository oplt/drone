...
def largest_viable_inward_offset(
    polygon: Polygon,
    *,
    ...
    for _ in range(16):
        mid = (lo + hi) / 2.0
        candidate = largest_polygon(polygon.buffer(-mid))
...
def ring_signed_area_xy(ring_xy: Sequence[tuple[float, float]]) -> float:
    if len(ring_xy) < 3:
        return 0.0
    area2 = 0.0
...
def is_clockwise_xy(ring_xy: Sequence[tuple[float, float]]) -> bool:
    return ring_signed_area_xy(ring_xy) < 0.0


def polyline_length_m(points_xy: Sequence[tuple[float, float]]) -> float:
    if len(points_xy) < 2:
        return 0.0
    total = 0.0
...
def coords_close(a: Coordinate, b: Coordinate, tol: float = 1e-7) -> bool:
    return abs(float(a.lat) - float(b.lat)) <= tol and abs(float(a.lon) - float(b.lon)) <= tol


def route_length_for_coords(coords: Sequence[Coordinate]) -> float:
    if len(coords) < 2:
        return 0.0
    ...
    return polyline_length_m(xy)


def dynamic_trigger_profile(*, ai_tasks: Sequence[str], path_offset_m: float) -> dict[str, Any]:
