...
async def _get_field(db, field_id: int, org_id: int | None):
    ...
    if org_id is not None and getattr(row, "org_id", None) is not None and row.org_id != org_id:
...
async def field_kml(
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="field_{field_id}.kml"'},
    )
...
async def mission_gpx(
    ...
    for rec in records:
        ...
        track_points.append(f'    <trkpt lat="{lat}" lon="{lon}">{ele_str}{time_str}</trkpt>')
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="mission_{mission_id}.gpx"'},
    )
...
async def fmis_push(
    ...
    from backend.modules.integrations.webhooks.events import dispatch_event
