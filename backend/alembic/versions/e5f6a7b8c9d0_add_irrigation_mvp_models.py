...
def upgrade() -> None:
    ...
    op.create_index("ix_processed_field_layers_mission_id", "processed_field_layers", ["mission_id"])
    op.create_index("ix_processed_field_layers_org_id", "processed_field_layers", ["org_id"])
    op.create_index("ix_processed_field_layers_project_id", "processed_field_layers", ["project_id"])
    ...
    op.create_table(
        ...
        sa.Column("zone_id", sa.Integer, sa.ForeignKey("anomaly_zones.id", ondelete="SET NULL"), nullable=True),
        sa.Column("org_id", sa.Integer, sa.ForeignKey("organizations.id", ondelete="SET NULL"), nullable=True),
        sa.Column("project_id", sa.Integer, sa.ForeignKey("projects.id", ondelete="SET NULL"), nullable=True),
        sa.Column("lat", sa.Float(), nullable=False),
        sa.Column("lon", sa.Float(), nullable=False),
