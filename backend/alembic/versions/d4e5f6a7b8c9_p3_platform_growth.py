...
def upgrade() -> None:
    ...
    op.create_table(
        "scheduled_runs",
        sa.Column("id", sa.Integer, primary_key=True),
        sa.Column("template_id", sa.Integer, sa.ForeignKey("mission_templates.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("idx_scheduled_run_template_time", "scheduled_runs", ["template_id", "created_at"])
    ...
    op.create_index("idx_webhook_delivery_endpoint_time", "webhook_deliveries", ["endpoint_id", "created_at"])
    op.create_index("idx_webhook_delivery_status_retry", "webhook_deliveries", ["status", "next_retry_at"])
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Integer, primary_key=True),
        sa.Column("org_id", sa.Integer, sa.ForeignKey("organizations.id", ondelete="SET NULL"), nullable=True),
        sa.Column("mission_runtime_id", sa.Integer, sa.ForeignKey("mission_runtimes.id", ondelete="CASCADE"), unique=True, nullable=False),
    ...
    op.create_index("idx_operator_cert_user_type", "operator_certifications", ["user_id", "cert_type"])
