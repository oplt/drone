...
import type { AnimalFarmMissionStatus, AnimalFarmWaypoint } from "../animalFarmPageTypes";

type AnimalFarmWaypointsStatusSectionProps = {
  waypoints: AnimalFarmWaypoint[];
  alt: number;
  missionStatus: AnimalFarmMissionStatus | null;
  activeFlightId: string | null;
};

export function AnimalFarmWaypointsStatusSection({
  waypoints,
  alt,
  missionStatus,
  activeFlightId,
}: AnimalFarmWaypointsStatusSectionProps) {
  return (
    <>
      {waypoints.length > 0 && (
        <Box sx={{ mt: 3 }}>
          ...
          <Stack spacing={1}>
            {waypoints.map((waypoint, idx) => (
              <Typography key={idx} variant="body2">
                {idx + 1}. Lat: {waypoint.lat.toFixed(6)}, Lon: {waypoint.lon.toFixed(6)}, Alt:{" "}
                {waypoint.alt ?? alt}m
              </Typography>
            ))}
          </Stack>
