...
function parseBoundary(value: string): Polygon {
  ...
  const geometry = (parsed.geometry as Record<string, unknown> | undefined) ?? parsed;
  if (geometry.type !== "Polygon" || !Array.isArray(geometry.coordinates)) throw new Error("Boundary must be a GeoJSON Polygon.");
...
export function AgricultureFieldBoundaryEditor({ fieldId }: { fieldId: number }) {
  ...
  const saveZone = () => { const lon = Number(zoneLon); const lat = Number(zoneLat); if (!Number.isFinite(lon) || !Number.isFinite(lat)) { setError("Zone longitude and latitude are required."); return; } setError(null); addZone.mutate({ fieldId, payload: { zone_type: zoneType, geometry: { type: "Point", coordinates: [lon, lat] }, name: zoneType === "exclusion" ? "Flight exclusion" : "Obstacle", kind: zoneType === "obstacle" ? "unknown" : "no-fly", radius_m: zoneType === "obstacle" ? 5 : null, height_m: null, metadata: {} } }); setZoneLon(""); setZoneLat(""); };
  ...
  return <Card component="section" variant="outlined" aria-labelledby="boundary-editor-title"><CardContent><Stack spacing={2}>
    <div><Typography id="boundary-editor-title" variant="h6">Field boundary and safety zones</Typography><Typography variant="body2" color="text.secondary">Edit or paste WGS84 GeoJSON. Every save creates a revision; self-intersections, unsupported CRS and zones outside the boundary are rejected.</Typography></div>
    ...
    <Typography id="boundary-help" variant="caption" color="text.secondary">Paste a Polygon or Feature containing a Polygon. Coordinates are [longitude, latitude].</Typography>
    ...
    <Stack direction={{ xs: "column", sm: "row" }} spacing={1}><TextField select label="Zone type" SelectProps={{ native: true }} value={zoneType} onChange={(event) => setZoneType(event.target.value as "exclusion" | "obstacle")}><option value="exclusion">Exclusion zone</option><option value="obstacle">Obstacle</option></TextField><TextField label="Longitude" inputMode="decimal" value={zoneLon} onChange={(event) => setZoneLon(event.target.value)} /><TextField label="Latitude" inputMode="decimal" value={zoneLat} onChange={(event) => setZoneLat(event.target.value)} /><Button variant="outlined" onClick={saveZone} disabled={addZone.isPending}>Add zone</Button></Stack>
