      ...
      <Stack spacing={1}>
        ...
        {alerts.data?.items.map((item) => (
          <Stack key={item.id} direction={{ xs: "column", sm: "row" }} spacing={1} alignItems={{ sm: "center" }}>
            ...
            {!item.assigned_to_user_id ? <Button size="small" onClick={() => { const value = window.prompt("Reviewer user id"); if (value) assign.mutate({ id: item.id, assigned_to_user_id: Number(value) }); }}>Assign</Button> : null}
