...
function OrgsTab() {
  const { data, isLoading, error } = useQuery({
    queryKey: ["admin-orgs"],
    queryFn: () => fetchAdminOrganizations<any>(),
  });
...
function MappingJobsTab() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => fetchAdminMappingJobs<any>(),
  });

  const requeue = useMutation({
    mutationFn: (jobId: number) =>
      requeueMappingJob(jobId),
...
function ExportJobsTab() {
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => fetchAdminExportJobs<any>(),
  });
...
function WorkerHealthTab() {
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () => fetchAdminWorkerHealth<any>(),
