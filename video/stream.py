...
class DroneVideoStream:
    ...
    def __init__(
        ...
        fps_limit: (
            float | None
        ) = None,  # throttle frame rate to save CPU/LLM cost (None = no limit)
    ...
    def _check_connection_health(self):
        ...
        if self.last_frame_time > 0:
            ...
            expected_interval = (
                1.0 / self.fps if self.fps > 0 else 0.033
            )  # Default to ~30fps
            if (
                frame_interval > expected_interval * 3
            ):  # Allow tolerance for occasional slow frames
                logging.warning(
                    f"Video stream frame rate degraded: {1.0 / frame_interval:.1f} fps (expected: {self.fps})"
                )
        ...
        if (
            self.last_frame_time > 0 and (now - self.last_frame_time) > 5.0
        ):  # 5 seconds without frames
            self.connection_healthy = False
    ...
    def frames(self) -> Iterator[tuple[int, Any]]:
        ...
        max_consecutive_failures = (
            10  # Allow up to 10 consecutive failures before giving up
        )
            ...
            if not ok:
                ...
                if not ok:
                    ...
                    if consecutive_failures >= max_consecutive_failures:
                        logging.error(
                            f"Failed to read frame from video source after {consecutive_failures} consecutive attempts - stopping video stream"
                        )
                        break
                    # Log warning only occasionally to reduce spam
                    if consecutive_failures % 5 == 0:
                        logging.warning(
                            f"Failed to read frame ({consecutive_failures}/{max_consecutive_failures} consecutive failures)"
                        )
    ...
    def _reconnect(self):
        ...
        try:
            ...
            was_recording = (
                self.video_writer is not None and self.video_writer.isOpened()
            )
            ...
            if self.cap and self.cap.isOpened():
                ...
                if was_recording:
                    ...
                    self._start_recording()
                    logging.info(
                        f"Resumed recording to existing file: {preserved_filename}"
                    )
            else:
                ...
                if was_recording and self.enable_recording:
                    self.recording_filename = preserved_filename
                    logging.warning(
                        "Reconnection failed, will retry recording when connection restored"
                    )
